...
void inicie_SPI(structspi* spi);
char sigo_enviando_SPI(structspi* spi);
void envie_dato(structspi* spi);
void actualize_valor(structspi* spi, unsigned int valorspi );
void seleccionar_impedancia(structspi* spi, char impedanciaspi);
void seleccionar_ganancia(structspi* spi, char gananciaspi);
void inicie_envio_SPI(structspi* spi);
void seleccionar_canal(structspi* spi,char canalspi);
# 49 "main.c" 2
...
struct usart{
    ...
    int numero3;
    int caso;
    _Bool recibir_dato_completo;
    _Bool esperar_datos_eusart;
};
...
void recibir_periodo_Eusart(usart *disp);
void enviar_Eusart(int valor);
char recibir_Eusart(usart *disp);
void parar_Eusart(usart *disp);
_Bool continuar_envio_Eusart(usart *disp);
_Bool dato_recibido_completo(usart *disp);
# 50 "main.c" 2


int primerresultado = 0;
int segundoresultado = 0;
char bandera_ADC1_start = 0;
char bandera_ADC2_start = 0;
char bandera_ADC1_finish = 0;
char bandera_ADC2_finish = 0;
...
# 77 "main.c"
    ...
    UART1_Initialize();
    Tm_Periodico timer;
    spi1_modes_t estructuraSpi=0;
    Tm_Inicie_periodico(&timer,1000);
    eCola cola;
    inicie_cola(&cola);

    structspi spi;
    inicie_SPI(&spi);
    actualize_valor(&spi,2048);
    usart disp;
...
# 106 "main.c"
        if(TMR2_HasOverflowOccured()){
            Tm_Procese_tiempo(&timer);
        }
        if(Tm_Hubo_periodico(&timer)){
            Tm_Baje_periodico(&timer);


            do { LATDbits.LATD4 = 1; } while(0);

            ADCC_StartConversion(primerADC);
            bandera_ADC1_finish = 1;
        }

        if(ADCC_IsConversionDone() && bandera_ADC1_finish){

            primerresultado = ADCC_GetConversionResult();

            ADCC_DischargeSampleCapacitor();

            ADCC_StartConversion(segundoADC);

            cola_add(&cola,(char)(primerresultado));
            do { LATDbits.LATD4 = 0; } while(0);



            bandera_ADC1_finish = 0;
            bandera_ADC2_finish = 1;

        }

        if(bandera_ADC2_finish && ADCC_IsConversionDone()){
            bandera_ADC2_finish = 0;
            segundoresultado = ADCC_GetConversionResult();
            segundoresultado = 3000;
            ADCC_DischargeSampleCapacitor();
            cola_add(&cola,segundoresultado & 0xFF);
            cola_add(&cola,((primerresultado & 0xF00) >> 4) | ((segundoresultado & 0xF00) >> 8 ));
...
# 165 "main.c"
        ...
        if(UART1_is_rx_ready()){
            ...
            recibir_periodo_Eusart(&disp);
        }
        if(dato_recibido_completo(&disp)){
            Tm_Inicie_periodico(&timer,1 + actualizar_recibido_Eusart(&disp));
        }
